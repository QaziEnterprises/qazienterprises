...
import { Package, Users, FileSpreadsheet, TrendingUp, ArrowDownRight } from "lucide-react";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Top 10 debtors (positive balance = they owe us)
    const debtors = recv
    ...
    // Breakdown: positive vs negative vs zero
  ...
  const cards = [
    ...
    { title: "Cash Sales", value: stats.totalCashSales, format: "currency", icon: FileSpreadsheet, link: "/sales", color: "text-success" },
  ...
  return (
    <div>
      ...
      {/* Charts Section */}
      {(topDebtors.length > 0 || breakdownData.length > 0) && (
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          ...
          {topDebtors.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Top 10 Debtors</CardTitle>
                </CardHeader>
                <CardContent>
          ...
          {/* Receivables Breakdown Pie */}
          {breakdownData.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Receivables Breakdown</CardTitle>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={320}>
                    <PieChart>
